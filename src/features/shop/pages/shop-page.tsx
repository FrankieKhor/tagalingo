				...
				<Card>
					<CardContent className="space-y-3 p-5">
						...
						<p className="text-4xl font-black text-slate-900 dark:text-white">
							{snapshot.profile.hearts}/5
						</p>
						<p className="text-sm leading-6 text-slate-600 dark:text-white/70">
							Use refill items only when you need them. They stay in inventory
							until then.
						</p>
					</CardContent>
				</Card>
				...
				<Card>
					<CardContent className="space-y-3 p-5">
						...
						<p className="text-2xl font-black text-slate-900 dark:text-white">
							{snapshot.equippedPowerUps.includes('streak-freeze')
								? 'Freeze armed'
								: 'No freeze armed'}
						</p>
						<p className="text-sm leading-6 text-slate-600 dark:text-white/70">
				...
				{shopCatalog.map((item) => {
					...
					return (
						<ShopItemCard
							...
							onToggleEquip={(itemId) => void toggleEquippedItem(itemId)}
						/>
					)
				})}
			</div>
