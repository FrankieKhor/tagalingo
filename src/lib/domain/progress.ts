...
function moveLeague(league: LeagueTier, delta: number) {
  return LEAGUE_ORDER[clampLeagueIndex(getLeagueIndex(league) + delta)] ?? league
}
...
function getDisplayedWeeklyXp(entry: LeaderboardEntry, cycle: LeaderboardCycle, now = new Date()) {
  if (!entry.isBot) {
...
function createPlayerLeaderboardEntry(snapshot: ProgressSnapshot, weeklyXp = 0): LeaderboardEntry {
  return {
...
function createLeaderboardBots(league: LeagueTier, seedKey: string) {
  ...
  for (let index = identities.length - 1; index > 0; index -= 1) {
    ...
    ;[identities[index], identities[randomIndex]] = [identities[randomIndex], identities[index]]
  }
...
function createLeaderboardCycle(snapshot: ProgressSnapshot, xpGain: number, now = new Date()): LeaderboardCycle {
  ...
  return {
    ...
    entries: [...createLeaderboardBots(snapshot.leaderboard.currentLeague, seedKey), playerEntry],
  }
}

function updateLeaderboardCyclePlayerXp(cycle: LeaderboardCycle, snapshot: ProgressSnapshot, xpGain: number) {
...
function resolveExpiredLeaderboardCycle(snapshot: ProgressSnapshot, now = new Date()) {
...
function applyLeaderboardXp(snapshot: ProgressSnapshot, xpGain: number, now = new Date()): ProgressSnapshot {
...
function useStreakFreezeIfNeeded(snapshot: ProgressSnapshot, today: string, dayDiff: number) {
  ...
  return {
    ...
    equippedPowerUps: snapshot.equippedPowerUps.filter((itemId) => itemId !== "streak-freeze"),
  }
}

export function hydrateDailyState(snapshot: ProgressSnapshot): ProgressSnapshot {
  const today = toISODate()
  const lastActive = snapshot.profile.lastActiveOn
  const needsQuestRefresh = snapshot.lastQuestRefreshOn !== today
  const questReadySnapshot = ensureQuestState(resolveExpiredLeaderboardCycle(snapshot), today)
  ...
  const streakState =
    dayDiff <= 0
      ...
      : dayDiff === 1
        ...
        : useStreakFreezeIfNeeded(questReadySnapshot, today, dayDiff)
  ...
  return syncQuestProgress({
    ...
    profile: {
      ...
      streak: dayDiff > 1 ? streakState.streak : questReadySnapshot.profile.streak,
      lastActiveOn: streakState.lastActiveOn,
      dailyGoal: {
        ...questReadySnapshot.profile.dailyGoal,
        currentXp: dayDiff > 0 ? 0 : questReadySnapshot.profile.dailyGoal.currentXp,
      },
    },
  })
}

function updateWeakWords(current: WeakWordStat[], words: string[]): WeakWordStat[] {
...
function isChestAlreadyOpened(snapshot: ProgressSnapshot, chestId: string) {
  ...
  const unit = curriculumUnits.find((entry) => getUnitChestId(entry.id) === chestId)
...
export function getPathChestState(
  ...
  const completedLessonsInUnit = unit.categories
    ...
    .filter((lesson) => snapshot.lessonProgress[lesson.id]?.status === "completed").length
...
export function getDailyChestState(snapshot: ProgressSnapshot, today = toISODate()) {
...
function rollChestReward(
  ...
  const rewardTable = source === "path" ? PATH_CHEST_REWARDS : DAILY_CHEST_REWARDS
  const availableTypes: Array<ChestReward["rewardType"]> =
    snapshot.profile.hearts >= MAX_HEARTS ? ["xp", "points"] : ["xp", "points", "hearts"]
...
function applyChestReward(snapshot: ProgressSnapshot, reward: ChestReward) {
  ...
  const updated = syncQuestProgress(applyLeaderboardXp({
    ...snapshot,
    profile: {
      ...snapshot.profile,
      xp: snapshot.profile.xp + xpGain,
      points: snapshot.profile.points + (reward.rewardType === "points" ? reward.amount : 0),
      hearts:
...
function createUnitItems(
          ...
          state: node.completed
            ...
            : node.isCurrent
              ...
              : node.unlocked
                ? "available"
                : "locked",
          ctaLabel: node.completed ? "Practice" : node.isCurrent ? "Start" : "Locked",
        })
      })
  ...
  const rewardedCategory = categories.find((category) => category.hasRewardChest) ?? categories[categories.length - 1]

  if (unitLessons.length > 0 && rewardedCategory) {
    const { chestId, afterLessonCount } = getUnitChestPlacement(unit)
    const anchorLesson = unitLessons[Math.min(afterLessonCount - 1, unitLessons.length - 1)]
...
export function buildLearnUnits(snapshot: ProgressSnapshot): LearnUnitViewModel[] {
...
function buildLeaderboardTiers(currentLeague: LeagueTier): LeaderboardTierViewModel[] {
...
export function buildLeaderboardView(
  ...
  const activeCycle = cycle && new Date(cycle.endsAt).getTime() > now.getTime() ? cycle : undefined
  ...
  return {
    ...
    timeRemainingLabel: activeCycle ? formatLeaderboardTimeRemaining(activeCycle.endsAt, now) : undefined,
    cycleProgress: activeCycle ? getCycleElapsedRatio(activeCycle, now) : 0,
    lastResult: buildLeaderboardResultView(resolvedSnapshot.leaderboard.lastResult),
  }
}
...
export function applyLessonSummary(
  ...
  const score = Math.round(
    (latestResults.filter((result) => result.correct).length / lesson.exercises.length) * 100
  )
  ...
  const currentStreak =
    snapshot.profile.lastActiveOn && differenceInDays(today, snapshot.profile.lastActiveOn) === 1
  ...
  const nextLessonProgress: LessonProgress = {
    ...
    bestScore: Math.max(snapshot.lessonProgress[lesson.id]?.bestScore ?? 0, score),
  ...
  const nextLessonId = score >= 70 ? allLessons[currentIndex + 1]?.id : lesson.id
  ...
  const updated = syncQuestProgress(applyLeaderboardXp({
    ...
    speakingAttempts: [
      ...
      ...results.flatMap((result) => (result.speakingAttempt ? [result.speakingAttempt] : [])),
    ],
    dailyStats: {
      lessonSessionsCompleted: snapshot.dailyStats.lessonSessionsCompleted + 1,
      ...
      bestCorrectStreak: Math.max(snapshot.dailyStats.bestCorrectStreak, getBestCorrectStreak(results)),
    },
    profile: {
      ...snapshot.profile,
  ...
  return {
    ...
    profile: {
      ...updated.profile,
      achievements: achievements.filter((id, index, arr) => arr.indexOf(id) === index),
    },
  }
}
...
export function applyReviewResult(
  ...
  const nextSnapshot = syncQuestProgress(applyLeaderboardXp({
    ...snapshot,
...
export function useInventoryItem(
  ...
  if ((snapshot.inventory[itemId] ?? 0) <= 0 || snapshot.profile.hearts >= MAX_HEARTS) {
    return snapshot
  }

  const heartsRestored = itemId === "heart-refill" ? MAX_HEARTS : Math.min(MAX_HEARTS, snapshot.profile.hearts + 2)
...
export function togglePowerUpEquip(
  ...
  if (isEquipped) {
    return {
      ...snapshot,
      equippedPowerUps: snapshot.equippedPowerUps.filter((entry) => entry !== itemId),
      inventory: {
        ...snapshot.inventory,
        [itemId]: (snapshot.inventory[itemId] ?? 0) + 1,
      },
