...
export function ExerciseRenderer({
	...
	useEffect(() => {
		window.setTimeout(() => setPendingMatchLeft(null), 0)
	}, [exercise.id])
	...
	function handleMatchRightSelect(rightValue: string) {
		...
		const existingLeft = Object.entries(nextAnswer).find(
								...
								<div className="flex gap-3">
									{showAudio ? (
										<Tooltip>
											...
											<TooltipContent>
												Uses browser speech playback in Filipino.
											</TooltipContent>
										</Tooltip>
									) : null}
										...
										<Tooltip>
											<TooltipTrigger asChild>
												<Button
													...
													onClick={onUseMic}
													disabled={
														!voiceCapability.recognitionAvailable || listening
													}
												>
							...
							{(exercise.type === 'tap-translation'
								...
								return (
									<TokenChip
										...
										onClick={() =>
											onChange(
												Array.isArray(answer) ? [...answer, token] : [token]
											)
										}
									/>
								)
							...
							<div className="space-y-3">
								{exercise.pairs.map((pair, index) => (
									<MatchCard
										...
										onClick={() =>
											setPendingMatchLeft((current) =>
												current === pair.left ? null : pair.left
											)
										}
									/>
								))}
									...
									return (
										<MatchCard
											...
											selected={
												...
												matchingAnswer[pendingMatchLeft] === option
											}
