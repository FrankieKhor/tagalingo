...
const toggleSettings = [
  ["soundEnabled", "Sound effects", "Play success and mistake cues during lessons."],
  ["speechPlaybackEnabled", "Speech playback", "Allow browser audio for phrases and prompts."],
  ["microphoneEnabled", "Microphone prompts", "Show speaking and dictation input options when available."],
  ["reducedMotion", "Reduced motion", "Tone down celebratory animations and motion-heavy transitions."],
] as const
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {(["light", "dark"] as const).map((theme) => (
            <Button
              key={theme}
              type="button"
              variant={snapshot.settings.theme === theme ? "default" : "outline"}
              className={cn(
      ...
      <div className="rounded-[28px] border border-slate-200 bg-white/80 p-4 dark:border-white/10 dark:bg-[#13212a]/80">
        <div className="mb-3">
          <p className="font-semibold text-slate-900 dark:text-white">Learning preferences</p>
          <p className="text-sm leading-6 text-slate-600 dark:text-white/70">
