    ...
    <Card>
      <CardContent className="space-y-4 p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm leading-6 text-slate-600 dark:text-white/70">{item.description}</p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between text-sm text-slate-600 dark:text-white/70">
          ...
          <span>{equipped ? "Armed now" : item.type === "power-up" ? "Manual arm" : "Manual use"}</span>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          {item.id === "streak-freeze" ? (
            ...
            <Button
              type="button"
              className="h-11 rounded-2xl bg-slate-900 text-white hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-white/90"
              onClick={() => onUse(item.id as Extract<ShopItemId, "heart-refill" | "heart-snack">)}
              disabled={!canUse}
            >
              Use now
            </Button>
