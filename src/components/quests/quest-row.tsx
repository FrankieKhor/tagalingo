...
function getQuestIcon(quest: Quest) {
  ...
  return quest.kind === "daily" ? Sparkles : ShieldCheck
}
      ...
      <CardContent className="space-y-4 p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-semibold text-slate-900 dark:text-white">{quest.title}</p>
              <p className="text-sm leading-6 text-slate-600 dark:text-white/70">{quest.description}</p>
            </div>
          </div>
